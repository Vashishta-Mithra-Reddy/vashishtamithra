...
export const ThemeToggleButton5 = ({
  ...
  const effectiveIsDark = isDark ?? localIsDark;
  const handleToggle = onToggle ?? (() => setLocalIsDark(!localIsDark));
  return (
    <button
      type="button"
      className={cn(
        "rounded-full transition-all duration-300 active:scale-95",
        effectiveIsDark ? "bg-black text-white" : "bg-white text-black",
        className,
      )}
    ...
    >
      ...
      >
        <clipPath id="skiper-btn-4">
          <motion.path
            animate={{
              y: effectiveIsDark ? 5 : 0,
              x: effectiveIsDark ? -20 : 0,
            }}
            transition={{ ease: "easeInOut", duration: 0.35 }}
